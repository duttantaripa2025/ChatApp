...
const Sidebar = () => {
  ...
  // ✅ Fix filter logic: users.filter not users.filter(users)
    ...
    >
      ...
      <div className="flex flex-col">
        {filteredUsers.map((user, index) => (
          <div
            onClick={() => {setSelectedUser(user); setUnseenMessages(prev =>({...ProgressEvent, [user._id]:0}))}}
            key={index}
          ...
          >
            ...
            <div className="flex flex-col leading-5">
              <p>{user.fullName}</p>
              {onlineUsers.includes(user._id) ? ( // ✅ use .includes not include
